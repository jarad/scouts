...
\title{FINAL Pack 196 Popcorn Report}
\author{Jarad Niemi}
\date{\today}
...
d <- readr::read_csv("popcorn.csv") %>%
  ...
  mutate(
    ...
    # Change 1 Bade's Blend Jug to Caramel Cyclone Jug
    count = ifelse(`Scout Name` == "Mellies, Oliver" & item == "Bade's Blend Jug",
                   count-1, count),
    count = ifelse(`Scout Name` == "Mellies, Oliver" & item == "Caramel Cyclone Jug",
                   count+1, count),
    
    # Change military count into amount
